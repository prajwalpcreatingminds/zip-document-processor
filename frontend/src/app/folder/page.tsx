...
export default function FolderPage() {
  ...
  const { file, folderName, setFolderName } = useWorkflow();
  const [error, setError] = useState<string | null>(null);

  const cleanDisplay = folderName.trim() || "job description";
  const originalZipName = file?.name || "archive.zip";
  ...
  const sampleSuggestions = ["requisition", "job description", "resume", "developer documents"];
      ...
      <div className="glass-card rounded-2xl p-8 sm:p-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[#64748B] text-sm mt-1.5 max-w-md mx-auto">
            Enter the folder name where the uploaded documents should be organized.
          </p>
        </div>
        ...
        <form onSubmit={handleNext}>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-semibold text-sm shadow-md shadow-blue-500/20 transition-all flex items-center gap-2 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
